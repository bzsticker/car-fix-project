...
export default function OwnerInvoicesPage() {
  ...
  const loadData = React.useCallback(async () => {
    ...
    try {
      ...
      let url = "/api/v1/invoices?limit=1000";
      if (filterBranchId) {
        url += `&filter_branch_id=${filterBranchId}`;
      }

      const res = await fetchApi<InvoicesResponse>(url);
      setInvoices(res.data ?? []);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to load database invoices index");
    } finally {
      setLoading(false);
    }
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-display text-4xl font-extrabold tracking-tight text-white">
            GLOBAL <span className="text-brand-red">INVOICES LEDGER</span>
          </h1>
          <p className="mt-1 text-sm text-muted">
            Global tracking of customer billing, outstanding payments, and multi-branch transaction audits.
          </p>
        </div>
      </div>
                ...
                <div className="border-b border-border pb-3">
                  <div className="flex items-center justify-between">
                    <div className="flex flex-col gap-1">
                      ...
                      <span className="rounded bg-white/5 px-2 py-0.5 text-xxs font-semibold uppercase tracking-wider text-muted border border-border self-start">
                        {branchMap.get(item.branch_id) ?? "HQ"}
                      </span>
                    </div>
                    
                    <span className={`text-[10px] font-extrabold uppercase tracking-wider px-2 py-0.5 rounded border self-start ${
                      item.status === "paid"
